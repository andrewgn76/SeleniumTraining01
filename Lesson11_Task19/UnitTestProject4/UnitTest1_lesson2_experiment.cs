...
using OpenQA.Selenium.Support.Events;       // нужно для работы с событиями (lesson 10)
using System.Drawing.Imaging;           // нужно для снятия скриншота, задание 17
...
namespace UnitTestProject4_lesson8
{
    [TestFixture]
    public class UnitTest1_lesson7_Task14         // проверка открытия новых окон
    {
        private IWebDriver driver;
        private WebDriverWait wait;
        private const bool ConsoleDebug = true; // признак вывода на консоль для отладки
        ...
        [Test]
        public void NewWindowOpenCheck()    // запуск теста 
        {
            Console.Write("Point 2.1. has reached; ");   // отладка
            // 1) зайти в админку
            LoginShopAdmin();            // вход в магазин  (админом)
            ExecuteRoutineT14();     // основные шаги тестового задания
            LogoutShopAdmin();         // выход из магазина 
            Console.Write("Point 2.2. has reached; ");   // отладка
        }

        public void ExecuteRoutineT14()    // основной тест 
        {   //Сделайте сценарий, который проверяет, что ссылки на странице редактирования страны открываются в новом окне.
            ...
            // 2) открыть пункт меню Countries(или страницу http://localhost/litecart/admin/?app=countries&doc=countries)
            // входим в магазин
            string CountriesMenuLink = "http://localhost/litecart/admin/?app=countries&doc=countries";
            driver.Url = CountriesMenuLink;
            if (ConsoleDebug) { Console.Write(" CountriesMenuLink opened;"); }; //  отладка

            // 3) открыть на редактирование какую-нибудь страну;
            string locPencilCountry = "#content tbody tr:nth-child(9) td:nth-child(7) i[class='fa fa-pencil']"; // Локатор элемента карандаш напротив страны Antarctida
            eCurr = driver.FindElement(By.CssSelector(locPencilCountry));   // находим этот карандаш для редактирования
            eCurr.Click();
            if (ConsoleDebug) { Console.Write(" Pencil clicked;"); }; //  отладка
            ...
            string loc7ExtLink = "#content table:nth-child(2) a[target='_blank']"; // локатор, который выдает 7 внешних ссылок
            IList<IWebElement> welExtLinks = driver.FindElements(By.CssSelector(loc7ExtLink)); // находим все элементы
            int iExtLinksCount = welExtLinks.Count;     // на всякий случай сохраняем количество эдементов в отдельную перменную
            if (ConsoleDebug) { Console.Write(" iExtLinksCount=" + iExtLinksCount.ToString() + ";"); };

            string sMainWindowHandle = driver.CurrentWindowHandle;     // запоминаем id текущего окна (handle)

            string sNewWinURL, sNewWinTargetURL;       // для ипользования в цикле
            ICollection<string> existingWindows;
            string sNewWindow;
           ...
           // закрыть его, вернуться обратно, и повторить эти действия для всех таких ссылок.
            ...
            for (int i = 0; i < iExtLinksCount; i++)   // последовательно перебираем все элементы
             {
                existingWindows = driver.WindowHandles;  // запоминаем все открытые окна

                // a. получаем ссылку, куда ведет квадратик со стрелкой
                sNewWinTargetURL = welExtLinks[i].GetAttribute("href");
                if (ConsoleDebug) { Console.Write(" NewWinURL[" + i.ToString() + "]=" + sNewWinTargetURL + ";"); }; //  отладка

                // b. открываем новое окно по полученной сылке
                welExtLinks[i].Click();
                if (ConsoleDebug) { Console.Write(" welExtLinks[" + i.ToString() + "] clicked;"); }; //  отладка

                // c. проверяем, что новое окно открылось и имеет такой же URL как и должно было + ожидание окна
                string sNewWinHandle = WaitUntilNewWindow(existingWindows, 30); // handle окна, если оно появилось или пустая строка если нет
                if (ConsoleDebug) { Console.Write(" WaitUntilNewWindow()=" + sNewWinHandle + ";"); }; //  отладка

                if ("" == sNewWinHandle) { throw new AssertFailedException(); }; // ломаем тест, если окно не появилось за заданное время, надо разбираться

                if (i != 3) { driver.SwitchTo().Window(sNewWinHandle); } // 4й элемент очень долго грузится, отключаем для отладки
                // driver.SwitchTo().Window(sNewWinHandle);   // эту строку использовать для быстрого соединения

                sNewWinURL = driver.Url;           

                string sHTTPS = sNewWinURL.Substring(0,5); // обрезем HTTPS, делаем HTTP
                if ("https" == sHTTPS) { sNewWinURL = "http" + sNewWinURL.Substring(5, (sNewWinURL.Length-5)); }; // заменяем  HTTPS на HTTP для последующей проверки

                sHTTPS = sNewWinTargetURL.Substring(0, 5);  // обрезем HTTPS, делаем HTTP
                if ("https" == sHTTPS) { sNewWinTargetURL = "http" + sNewWinTargetURL.Substring(5, (sNewWinTargetURL.Length - 5)); }; // заменяем  HTTPS на HTTP для последующей проверки

                // проверяем, что открылась страница с ожидаемым URL:
                if (sNewWinTargetURL == sNewWinURL)
                {
                    if (ConsoleDebug) { Console.Write(" NewWinURL[" + i.ToString() + "] OK;"); }; //  отладка
                }
                else
                {
                    if (ConsoleDebug) {
                    Console.Write(" NewWinURL[" + i.ToString() + "] FAILED;");
                    Console.Write(" sNewWinTargetURL=" + sNewWinTargetURL + ";");
                    Console.Write(" sNewWinURL=" + sNewWinURL + ";");
                    }; //  отладка
                }

                CloseAllWindowExceptMain(sMainWindowHandle);   // закрываем все окна, кроме главного (т.е. новое)
                driver.SwitchTo().Window(sMainWindowHandle);    // возвращаемся на пред.окно (вкладку). Обязательный шаг, иначе вылетит исключение
            };

            Console.Write(" All Windows are processed;");   // отладка

            // d. закрываем 
            // для интереса делаем так - сначала 7 окон последовательно открываем, не закрывая предыдущее, а затем их закрываем.
            // CloseAllWindowExceptMain(sMainWindowHandle);
            // driver.SwitchTo().Window(sMainWindowHandle); // возвращаемся обратно в главное окно
            //if (true) { throw new AssertFailedException(); };      //принудительно ломаем тест
        ...
        public void CloseAllWindowExceptMain(string sMainWindowHandle)
        {
            ICollection<string> WinsOpened = driver.WindowHandles;    // для цикла
            if (ConsoleDebug) { Console.Write(" WinsOpened.Count=" + WinsOpened.Count.ToString() + ";"); }; //  отладка

            foreach (string hWin in WinsOpened)
            {
                if (hWin != sMainWindowHandle)
                {
                    driver.SwitchTo().Window(hWin);
                    driver.Close(); }  // закрываем окно, если handle другой
                };
            driver.SwitchTo().Window(sMainWindowHandle);

        }

        public string WaitUntilNewWindow(ICollection<string> oldWindows, int timeoutSec)  // true если новое окно появилось
        {
           ICollection<string> currWins;    // для цикла
           ICollection<string> deltaWins;

           deltaWins = new List<string>();
           // if (ConsoleDebug) { Console.Write(" deltaWins(before)=" + deltaWins.Count.ToString() + ";"); }; //  отладка


            for (int i = 0; i<timeoutSec; i++)     // упрощенная версия цикла, засыпаем на 1 сек
            {
                currWins = driver.WindowHandles;    // получаем список окон
              //  if (ConsoleDebug) { Console.Write(" deltaWins[" + i.ToString() + "]=" + deltaWins.Count.ToString() + ";"); }; //  отладка

                foreach (string sNewWin in currWins)
                {
                    bool bIsNotFound = true;
                    foreach (string sOldWin in oldWindows)
                    {
                        if (sNewWin == sOldWin)    // если элемент найден, то ничего не делаем
                        {
                            bIsNotFound = false;
                            //break;
                        }
                    }
                    if (bIsNotFound) { deltaWins.Add(sNewWin); };
               ...
               // if (ConsoleDebug) { Console.Write(" deltaWins(after)=" + deltaWins.Count.ToString() + ";"); }; //  отладка
                if (deltaWins.Count > 0)         // что-то новое нашлось
                {
                    foreach (string sWin in deltaWins)  // проход по членам колеекции
                    {
                        return sWin; // возвращается первый найденный элемент
                    }
                }
                else
                {
            ...
            return "";      //возвращаем пустую строку, если ничего не нашлось за указанное время
...
namespace UnitTestProject4_lesson9
{
    [TestFixture]
    public class UnitTest1_lesson9_Task15         // исспользование грида, развернутого на локальной машине, к которому подключены машины виртуальные
    {
        //private IWebDriver driver;
        private WebDriverWait wait;
        private const bool ConsoleDebug = true; // признак вывода на консоль для отладки

        private IWebDriver driverCH;
        ...
        public void Start()
        {
            ...
            string sHubAddress = "http://localhost:3333/wd/hub";  // по этому адресу ДОЛЖЕН быть запущен Selenium Grid
            driverCH = new RemoteWebDriver(new Uri(sHubAddress), DesiredCapabilities.Chrome());    // 
            driverIE = new RemoteWebDriver(new Uri(sHubAddress), DesiredCapabilities.InternetExplorer()); // 
            driverFF = new RemoteWebDriver(new Uri(sHubAddress), DesiredCapabilities.Firefox());   //
        ...
        [Test]
        public void GridTestCheck()    // запуск теста 
        {
            Console.Write("Point 2.1. has reached; ");   // отладка
            ExecuteRoutineT15();     // основные шаги тестового задания
            Console.Write("Point 2.2. has reached; ");   // отладка
        }

        public void ExecuteRoutineT15()    // основной тест 
        {   //Задание 15. Постройте небольшой грид
            ...
            Console.Write(" 3 browsers are started; ");
            RunYandexTest(driverCH);    // последовательно вызываем тесты и смотрим, что происходит на локальной и виртуальной машине
            Console.Write(" Chrome completed; ");

            RunYandexTest(driverIE);
            Console.Write(" IE completed; ");

            RunYandexTest(driverFF);
            Console.Write(" FF completed; ");

            //pauseMY(20); // для отладки
        ...
        private void RunYandexTest(IWebDriver drv)        // простой тест - открыть Яндекс, забить туда поисковую строку 
        {
            drv.Url = "http://www.ya.ru";       // идем на Яндекс
            string locSS = "input#text.input__control.input__input"; // локатора строки поиска
            IWebElement we = drv.FindElement(By.CssSelector(locSS));
            we.SendKeys("This is the typing test from SELENIUM");
            we.SendKeys(Keys.Enter); 

            //string locBtnSearch = "body  div.search2__button > button"; // локатор кнопки поиска
            //we = drv.FindElement(By.CssSelector(locBtnSearch));
            //we.Click();

            pauseMY(10);  // для отладки - посмотреть, как забивается
        }
    ...
    [TestFixture]
    public class UnitTest1_lesson9_Task16         // запуск теста в облаке
    {
        private IWebDriver driver;
        private WebDriverWait wait;
        private const bool ConsoleDebug = true; // признак вывода на консоль для отладки
        ...
        public void Start()
        {
            ...
            string sCloudHubAddress = "http://hub-cloud.browserstack.com/wd/hub/";  // по этому адресу ДОЛЖЕН быть запущен Selenium Grid в ОБЛАКЕ

            // Прописываем capabilites (пример взят с сайта https://automate.browserstack.com/?utm_campaign=onboarding&utm_medium=email&utm_source=welcome)
            ...
            capability.SetCapability("browserstack.user", "andreigromov1");
            capability.SetCapability("browserstack.key", "vpqr5PTJ8DXZeuK1ERR3");
            capability.SetCapability("browserstack.debug", "true"); // добавлено для просмотра видео
 

            driver = new RemoteWebDriver(new Uri(sCloudHubAddress), capability);    // 
        ...
        [Test]
        public void CloudTestCheck()    // запуск теста 
        {
            Console.Write("Point 2.1. has reached; ");   // отладка
            ExecuteRoutineT16();     // основные шаги тестового задания
            Console.Write("Point 2.2. has reached; ");   // отладка
        }

        public void ExecuteRoutineT16()    // основной тест 
        {   
            //  Задание 16.Научитесь использовать облачный грид
            // Запустить несколько тестов в каком-нибудь облачном сервисе на выбор:
            // https://www.browserstack.com/
            ...
            Console.Write(" Cloud test is started;");
            RunYandexTest(driver);    // последовательно вызываем тесты и смотрим, что происходит на локальной и виртуальной машине
            Console.Write(" Cloud test is completed;");
...
namespace UnitTestProject4_lesson10
{
    [TestFixture]
    public class UnitTest1_lesson10_Task17         // проверка логов в браузере
    {
        ...
        public void Start()
        {
            ...
            driver.FindingElement += (sender,e) => Console.WriteLine(e.FindMethod); // здесь используем лямбда-выражение 
            driver.FindElementCompleted += (sender, e) => Console.WriteLine(e.FindMethod + " has found;"); // здесь используем лямбда-выражение 
            driver.ExceptionThrown += (sender, e) => Console.WriteLine(e.ThrownException);  // опять используем лямбда-выражение
        ...
        private void Driver_ExceptionThrown(object sender, WebDriverExceptionEventArgs e) // автовставка
        {
            throw new NotImplementedException();
        }

        private void Driver_FindingElement(object sender, FindElementEventArgs e)  // автовставка
        {
        ...
        [Test]
        public void CheckBrowserLog()    // запуск теста 
        {
            Console.Write("Point 2.1. has reached; ");   // отладка
            LoginShopAdmin();            // вход в магазин  (админом)
            ExecuteRoutineT17();     // основные шаги тестового задания
            LogoutShopAdmin();         // выход из магазина 

            Console.Write("Point 2.2. has reached; ");   // отладка
        }

        public void ExecuteRoutineT17()    // основной тест 
        {
            ...
            // здесь мы уже находимся в админке магазина  1) выполнено
            // безусловное снятие скриншота:  (просто проверка, как это работает)
            // TakeScreenshotMY();
            string sCatalogURL = "http://localhost/litecart/admin/?app=catalog&doc=catalog&category_id=1";
            driver.Url = sCatalogURL;

            // 3) последовательно открывать страницы товаров и проверять, не появляются ли в логе браузера сообщения(любого уровня)
            // получаем список товаров 
            string locGoods = "#content table tr td:nth-child(3) > a"; // локатор для поиска списка товаров
            IList<IWebElement> weGoodsList = driver.FindElements(By.CssSelector(locGoods));
            int iGoodsNumber = weGoodsList.Count;
            string sOneGoodURL;

            Console.WriteLine(" iGoodsNumber=" + iGoodsNumber.ToString() + "; ");
            //TakeScreenshotMY(); // отладка

            for (int i = 0; i < iGoodsNumber; i++)
            {
                weGoodsList = driver.FindElements(By.CssSelector(locGoods));    // заново получаем список элементов, т.к. он теряется после клика на странице
                sOneGoodURL = weGoodsList[i].GetAttribute("href");              // получаем ссылку на очередной товар
                driver.Url = sOneGoodURL;                                       // переходим на страницу по новой ссылке

                //TakeScreenshotMY(); // отладка

                // .. и проверять, не появляются ли в логе браузера сообщения(любого уровня)
                // (смотрим только на логи браузера)

                IList<LogEntry> leLogs = driver.Manage().Logs.GetLog("browser");
                
                if (0 == leLogs.Count)
                {
                    Console.WriteLine(" Duck[" + i.ToString() + "] - NO LOG found;");
                }
                else { 

                    foreach (LogEntry l in leLogs)
                    {
                        Console.WriteLine(" Duck[" + i.ToString() + "] log:");
                        Console.WriteLine(l);
                    }
                }

                driver.Url = sCatalogURL;   // возвращаемся обратно в каталог. Список элементов надо перезапросить
            }
        ...
        private void TakeScreenshotMY()
        {             
            string NameUniq = DateTime.Now.ToString();  //предполагаем, что время будет уникальным при вызоые этой функции, т.е. ее вызов реже чем раз в 1 сек
            NameUniq = NameUniq.Replace(":", ""); // удаляем ненужные символы
            NameUniq = NameUniq.Replace("/", "");
            NameUniq = NameUniq.Replace(".", "");
            NameUniq = NameUniq.Replace(",", "");
            ...
            string ssPath = @"C:\Users\SeaRu\source\repos\UnitTestProject4\SeleniumSS\screen_" + NameUniq + ".png";
    ...
    [TestFixture]
    public class UnitTest1_lesson10_Task18        // перенаправление траффка в прокси-сервер 
    {
        private IWebDriver driver;
        private WebDriverWait wait;
        private const bool ConsoleDebug = true; // признак вывода на консоль для отладки
        ...
        public void Start()
        {
            ...
            // установлен и запущен прокси Telerik на порту 8888 на той же самой машине
        ...
        [Test]
        public void CheckProxyTraffic()    // запуск теста 
        {
            Console.Write("Point 2.1. has reached; ");   // отладка
            LoginShopAdmin();            // вход в магазин  (админом)
            ExecuteRoutineT18();     // основные шаги тестового задания
            LogoutShopAdmin();         // выход из магазина 

            Console.Write("Point 2.2. has reached; ");   // отладка
        }

        public void ExecuteRoutineT18()    // основной тест 
        {
            ...
            // Используются шаги Задания 17. Проверьте отсутствие сообщений в логе браузера
            // Сделайте сценарий, который проверяет, не появляются ли в логе браузера сообщения при открытии страниц в учебном приложении, а именно --страниц товаров в каталоге в административной панели.
            // Сценарий должен состоять из следующих частей:
            // 1) зайти в админку
            // 2) открыть каталог, категорию, которая содержит товары(страница http://localhost/litecart/admin/?app=catalog&doc=catalog&category_id=1)
...
namespace UnitTestProject4_lesson11
{
    [TestFixture]
    public class UnitTest1_lesson11_Task19         // учебное задание 19, добваление/удаление нового товара в корзину c использованием многослойной архитектуры
    {
        private IWebDriver driver;
        private WebDriverWait wait;
        private const bool ConsoleDebug = true; // признак вывода на консоль для отладки
        ...
        [Test]
        public void AddRemoveGoods_WRONG()    // запуск теста 
        {
            Console.Write("Point 2.1. has reached; ");   // отладка
            //LoginShopAdmin();            // вход в магазин  (админом)
            ExecuteRoutineT19();     // основные шаги тестового задания
            //LogoutShopAdmin();         // выход из магазина 
            Console.Write("Point 2.2. has reached; ");   // отладка
        }

        public void ExecuteRoutineT19()    // основной тест 
        {
            ...
            // многослойную архитектуру. А именно, выделите вспомогательные классы для работы с главной страницей(откуда выбирается товар), для работы 
            // со страницей товара(откуда происходит добавление товара в корзину), со страницей корзины(откуда происходит удаление), и реализуйте 
            // сценарий, который не напрямую обращается к операциям Selenium, а оперирует вышеперечисленными объектами-страницами.

            // Задание 13. Сделайте сценарий для добавления товаров в корзину и удаления товаров из корзины.
            // 1) открыть главную страницу
            // 2а) открыть первый товар из списка
            // 2б) добавить его в корзину(при этом может случайно добавиться товар, который там уже есть, ничего страшного)
            // 3) подождать, пока счётчик товаров в корзине обновится
    ...
    public class LoginPage // вспомогательный класс для работы с страницей логина
    {
        /*
        public void LoginShopAdmin()    //Вспомогательный метод - логин админом в тестовый магазин, затем логаут
        {
            Console.Write(" LoginShopAdmin has entered;");   // отладка
                                                         // открытие  веб-магазина и логин в него:
                                                         // driver.Url = "http://localhost/litecart/en/";  // обычный магазин
            driver.Url = "http://localhost/litecart/admin/login.php"; // открытие админской консоли веб-магазина и логин в него:
            driver.FindElement(By.Name("username")).SendKeys("admin");
            driver.FindElement(By.Name("password")).SendKeys("admin");
            driver.FindElement(By.Name("login")).Click();
            Console.Write("LoginShopAdmin has completed;");   // отладка
